...
| Device | File | How |
|---|---|---|
| TX tester | `LPM-10A-TX_PN2.14-tone-recovery.bin` | tester off → hold **M + Power** → plug USB-C → copy onto the drive → long-press Power, power on |
...
`SHA256SUMS.txt` has both checksums; `TX-PN2.14-README.txt` and `RX-PN1.23-README.txt` say what each build
...
|---|---|
| `sdk/` | TX toolkit: patches, assembler, verifier, Thai UI, tests (`python build.py --write`) |
...
| `archive/` | earlier release copies and their per-version notes (TX PN 2.4–2.9, RX PN 1.0–1.8) — history only |
