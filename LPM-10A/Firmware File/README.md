...
| Device | File | How |
|---|---|---|
| TX tester | `LPM-10A-TX_PN2.21-cable-values.bin` | tester off → hold **M + Power** → plug USB-C → copy onto the drive → long-press Power, power on |
...
`SHA256SUMS.txt` has both checksums; `TX-PN2.21-README.txt` and `RX-PN1.23-README.txt` say what each build
...
|---|---|
| `sdk/` | TX toolkit: patch chain PN 2.9 → 2.21 (`profiles.py`), assembler, verifier, Thai UI, tests (`python build.py --write` = the latest profile) |
...
| `archive/` | earlier release copies and their per-version notes (TX PN 2.4–2.9, 2.14, 2.20, RX PN 1.0–1.8) — history only |
