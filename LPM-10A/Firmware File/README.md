...
| Device | File | How |
|---|---|---|
| TX tester | `LPM-10A-TX_PN2.26-qc-display.bin` | tester off → hold **M + Power** → plug USB-C → copy onto the drive → long-press Power, power on |
...
`SHA256SUMS.txt` has both checksums; `TX-PN2.26-README.txt` and `RX-PN1.24-README.txt` say what each build
does and how to roll back. The RX procedure in full (Thai + English): [`../../docs/RX-UPDATE-GUIDE.md`](../../docs/RX-UPDATE-GUIDE.md).

The owner reports **all PN2.26 functions passed on the tester, 2026-09-22**.
...
|---|---|
| `sdk/` | TX toolkit: patch chain PN 2.9 → 2.26 (`profiles.py`), assembler, verifier, Thai UI, tests (`python build.py --write` = the latest profile) |
...
| `archive/` | earlier release copies and their per-version notes (TX PN 2.4–2.9, 2.14, 2.20, 2.21, 2.23, RX PN 1.0–1.8, 1.23) — history only |
